...
func (tba *TokenBearerAuthenticator) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	err, code := checkTokenBearerCanDebugPFPs(context.Background(), tba.cs, tba.lh, r.Header.Get("Authorization"))
	if err != nil {
...
func checkTokenBearerCanDebugPFPs(ctx context.Context, cs kubernetes.Interface, lh logr.Logger, authHeader string) (error, int) {
